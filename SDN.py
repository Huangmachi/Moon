...
class ShortestForwarding(app_manager.RyuApp):
	...
	def get_path(self, src, dst):
		...
		"""
		best_paths = self.awareness.best_paths
		graph = self.awareness.graph
		paths = best_paths[src][dst]
		best_path = paths[0][0]

		if paths[0][2] == 1:
			print "Got best paths:"
		else:
			print "No best paths exist!"
			print "Reference paths are:"

		for i in xrange(len(paths)):
			print "%d. %s (Total_Cost: %d, Total_Weight: %s)" % (i+1, paths[i][0], paths[i][1], paths[i][2])
	...
	def install_flow(self, datapaths, link_to_port, access_table, path, flow_info, buffer_id, data=None):
		...
		# Src and Dst under the same datapath
		else:
			out_port = self.get_port(flow_info[2], access_table)
			if out_port is None:
				self.logger.info("Out_port is None under the same datapath.")
				return
			self.send_flow_mod(first_dp, flow_info, in_port, out_port)
			self.send_packet_out(first_dp, buffer_id, in_port, out_port, data)
